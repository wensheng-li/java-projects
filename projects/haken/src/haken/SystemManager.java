...
public class SystemManager {
	...
	public void statusSetting() {
		...
		sts1 = new ConsoleStatus(
				...
				"  終了 (X) \n" + 
				"_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/\n",
				"どの機能を実行しますか？[S, JI, JU, JD, KI, KD, X]＞", false);
		// 軌道から”S”入力時の状態
		sts2 = new ConsoleStatus("検索方法を指定してください。" + 
				"N ー＞氏名から検索　　Tー＞職種から検索\n" + 
				"Eー＞従業員検索終了（メニューに戻る）", "[N. T. E]＞",
				false);
		// 従業員IDを入力すると情報を表示する状態
		sts5 = new DisplayPersonStatus("", "エンターキーを押すと検索結果一覧に戻ります。\n＞", wlist, false);
		sts5_2 = new DisplayPersonStatus("", "エンターキーを押すと検索結果一覧に戻ります。\n＞", wlist, false);

		// 職種から得た従業者リストを表示し、従業員IDを入力する状態
		sts4 = new DisplayPersonByTypeStatus("", "E ー＞検索結果一覧終了（検索条件指定に戻る）［(従業員 ID), E］＞", false, plist, sts5);

		// 職種名を入力する状態
		sts3 = new TypeSelectionStatus("職種名を入力してください。", "[（職種名）]＞", false, sts4);

		// 氏名から得た従業者リストを表示し、従業員IDを入力する状態
		sts7 = new DisplayPersonsByNameStatus("", "E ー＞検索結果一覧終了（検索条件指定に戻る）［(従業員 ID), E］＞", false, plist, sts5_2);

		// 職種名を入力する状態
		sts6 = new NameSelectionStatus("氏名名を入力してください。", "[（氏名）]＞", false, sts7);

		// 従業員を追加する状態
		sts8 = new AddPersonStatus("", "エンターキーを押すとメニューに戻ります。＞", false, plist);

		// 従業員の情報を更新する状態
		sts9 = new UpdatePersonStatus("", "更新しました。エンターキーを押すとメニューに戻ります。＞", false, plist);

		// 従業員を削除する状態
		sts10 = new DeletePersonStatus("", "この従業員情報を削除しますか？（Y　はい　N　いええ）[Y, N]＞", false, plist);

		// 稼働を追加する状態
		sts11 = new AddWorkStatus("", "エンターキーを押すとメニューに戻ります。＞", false, clist, wlist);

		// 稼働を削除する状態
		sts12 = new DeleteWorkStatus("", "この稼働情報を削除しますか？（Y　はい　N　いええ）[Y, N]＞", false, plist, wlist);
