...
public class AddWorkStatus extends ConsoleStatus {
	 ...
	 * @param WorkList wl
	 */
	AddWorkStatus(String firstMsg, String promptMsg, boolean IsEndStatus,
			ClientList cl, WorkList wl) {
	...
	public void displayFirstMsg () throws Exception {
		...
		try {
			int pid = Integer.parseInt(data[0]); // 従業員のID
			int cid = Integer.parseInt(data[1]); // 顧客ID
			int pr = Integer.parseInt(data[4]); // 契約単価
			
			// 現在のWorkListのレコード数を新しいレコードのIDとする
			Work new_w = new Work (wl.size(), pid, cid, data[2], data[3], pr, false);
